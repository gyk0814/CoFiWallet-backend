...
const getCIByPhoneNumber = catchAsync(async (req, res) => {
  ...
  if (responseBody) {
    return res
      .status(200)
      .json({ message: "Successful authentication", CI: responseBody.CI });
  } else {
...
const changePassword = catchAsync(async (req, res) => {
  ...
  const change = await userServices.changePassword(
    id,
    existingPassword,
    newPassword
  );
...
const updateProfileImage = catchAsync(async (req, res) => {
  upload.single("image")(req, res, async function (err) {
    ...
    const { id } = req.user;
    const profileImage = await userServices.updateProfileImageURL(
      id,
      uploadedFileURL
    );
